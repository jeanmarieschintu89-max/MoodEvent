...
public final class EventAdvancedGUI {
    ...
    public static void open(Player player) {
        ...
        inv.setItem(4, EventItem.glow(EventItem.item(
                ...
                MoodStyle.detail("À utiliser pour dépannage ou event libre."),
        ...
        inv.setItem(11, EventItem.item(Material.NAME_TAG, "§6✦ §fType §6✦", MoodStyle.detail("Actuel : " + EventManager.getType().getDisplayName()), MoodStyle.detail("Change le type manuellement"), "", MoodStyle.info("Changer")));
        ...
        inv.setItem(15, EventItem.item(Material.RED_WOOL, "§6✦ §fArrivée manuelle §6✦", MoodStyle.detail("État : " + state(EventManager.hasFinishLocation())), MoodStyle.detail("Course, Jump, Water Jump, Labyrinthe"), "", MoodStyle.success("Définir ici")));
        inv.setItem(16, EventItem.item(WaitingRoomManager.hasRoom() ? Material.ENDER_EYE : Material.DARK_OAK_PLANKS, "§6✦ §fSalle d'attente manuelle §6✦", MoodStyle.detail("État : " + state(WaitingRoomManager.hasRoom())), MoodStyle.detail("Ancien système séparé"), "", MoodStyle.info("Ouvrir")));
