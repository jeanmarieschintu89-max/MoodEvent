...
public final class GeneratedVerticalJumpBuilder {
    ...
    public static Layout build(Location center, int platforms) {
        ...
        if (world == null) return new Layout(center, center, false, 0, 0, center.getY());
    ...
    private static Node adapt(Node previous, Node wanted, int cx, int cz) {
        ...
        int x = clamp(wanted.x(), previous.x() - maxMove, previous.x() + maxMove);
        int z = clamp(wanted.z(), previous.z() - maxMove, previous.z() + maxMove);
        int y = Math.min(wanted.y(), previous.y() + MAX_SAFE_Y_UP);
    ...
    private static void buildNode(World world, Node node, int index, boolean finish) {
        platform(world, node.x(), node.y(), node.z(), node.radius(), materialFor(index, finish));
        if (index > 0 && index % 9 == 0 && !finish) addLadderHint(world, node.x() + node.radius() + 1, node.y(), node.z(), BlockFace.WEST);
        if (index > 0 && index % 12 == 0 && !finish) fenceBack(world, node.x(), node.y(), node.z(), node.radius());
    }
    ...
    private static void fenceBack(World world, int cx, int y, int cz, int radius) {
        for (int x = cx - radius; x <= cx + radius; x++) world.getBlockAt(x, y + 1, cz - radius).setType(Material.OAK_FENCE, false);
    }
    ...
    public record Layout(Location start, Location finish, boolean reachable, int corrections, int platformCount, double fallY) {}
}
