...
public final class GeneratedLabyrinthBuilder {
    ...
    public static Layout build(Location center, int rawSize) {
        ...
        for (int x = minX - clear; x <= maxX + clear; x++) {
            for (int z = minZ - clear; z <= maxZ + clear; z++) {
        ...
        for (int gx = 0; gx < size; gx++) {
            for (int gz = 0; gz < size; gz++) {
                ...
                if (!path[gx][gz]) {
                    for (int y = cy; y <= cy + WALL_HEIGHT - 1; y++) world.getBlockAt(wx, y, wz).setType(Material.POLISHED_BLACKSTONE_BRICKS, false);
                }
            }
        }
    ...
    private static int sasRadius(int mazeSize) {
        return mazeSize >= 43 ? 4 : 3;
    }
    ...
    private static void buildCornerLights(World world, int minX, int maxX, int minZ, int maxZ, int cy) {
        int[][] corners = {{minX, minZ}, {maxX, minZ}, {minX, maxZ}, {maxX, maxZ}};
        for (int[] point : corners) world.getBlockAt(point[0], cy + 3, point[1]).setType(Material.SEA_LANTERN, false);
    }
