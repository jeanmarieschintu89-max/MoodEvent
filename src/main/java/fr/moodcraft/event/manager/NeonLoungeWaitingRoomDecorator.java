...
public final class NeonLoungeWaitingRoomDecorator {
    ...
    public static void decorate(World world, int cx, int cy, int cz, int radius, int height) {
        ...
        buildGlowCorners(world, cx, cy, cz, radius, height);
    }
    ...
    private static void buildDisplayFrames(World world, int cx, int cy, int cz, int radius) {
        ...
        for (int x = cx - 3; x <= cx + 3; x += 3) {
            world.getBlockAt(x, cy + 2, z).setType(Material.GLOW_ITEM_FRAME, false);
    ...
    private static void buildGlowCorners(World world, int cx, int cy, int cz, int radius, int height) {
        int[][] corners = {
